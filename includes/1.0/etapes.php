...
$app->get('/api/1.0/etapes', function ($req, $resp, $args) {   // ADAPT
   try {
      ...
      $etapes[] = ['semaine 1' => 'signaux'];
      $etapes[] = ['semaine 22' => 'ipc'];

      etapes[]= ['Sed'];
      $ret = array(
         'etapes' => (array)$etapes, // on (caste) en "array" par sécurité
      );
      /*** FIN_ADAPT */
      return buildResponse($resp, $ret);
   } catch (Exception $e) {
      __logException('Pb GET etapes', $e);   // ADAPT
...
$app->get('/api/1.0/etapes/{id}', function ($req, $resp, $args) {   // ADAPT
   $id = $args['id'];
   try {
      /** SECURITY CHECK - MANDATORY */
      if (!checkToken()) {
      ...
      if (!checkRecordExists($id)) {
         __log('Pb GET etapes #' . $id . ' - Inconnu');    // ADAPT
      ...
      __logException('Pb GET etapes #' . $id, $e);    // ADAPT
      return $resp->withStatus(500);   // Internal Server Error
   }
});
