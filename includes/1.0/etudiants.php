...
namespace Etudiants;    // ADAPT

function checkRecordExists($id) {
   // Pas de try...catch, c'est déjà fait dans l'appelant...
   $pdo = getPDO();
   /*** ADAPT */
   // $stmt = $pdo->prepare('SELECT 1 FROM `etudiants` WHERE `id` = :id');
   /*** FIN_ADAPT */
...
$app->get('/api/1.0/etudiants', function ($req, $resp, $args) {   // ADAPT
   try {
      ...
      // $etudiants[] = ...// Ajout d'un élément à un tableau
      $etudiants[] = [
         'nom' => 'Toto',
         'prenom' => 'Lulu',
         'matieres' => ['algo', 'sys', 'bdd']
      ];
      $etudiants[] = [
         'nom' => 'Mimi',
         'prenom' => 'Lili',
         'matieres' => ['math', 'francais', 'english']
      ];

      $ret = array(
         'etudiants' => (array)$etudiants, // on (caste) en "array" par sécurité
      );
      /*** FIN_ADAPT */
      return buildResponse($resp, $ret);
   } catch (Exception $e) {
      __logException('Pb GET etudiants', $e);   // ADAPT
...
$app->get('/api/1.0/etudiants/{id}', function ($req, $resp, $args) {   // ADAPT
   $id = $args['id'];
   try {
      /** SECURITY CHECK - MANDATORY */
      if (!checkToken()) {
      ...
      if (!checkRecordExists($id)) {
         __log('Pb GET etudiants #' . $id . ' - Inconnu');    // ADAPT
      ...
      __logException('Pb GET etudiants #' . $id, $e);    // ADAPT
      return $resp->withStatus(500);   // Internal Server Error
   }
});
